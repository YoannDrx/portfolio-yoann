/**
 * Site Configuration
 * Configuration globale du portfolio
 */

import type { SiteConfig } from "./types";
import type { Locale } from "@/i18n/locales";

const baseSiteConfig = {
  name: "Portfolio Yoann",
  url: "https://yoann-andrieux.fr",
  ogImage: "/og-image.png",
  theme: {
    primaryColor: "#2457E6",
    accentColor: "#B85C3A",
  },
  contact: {
    email: "yoann.andrieux@gmail.com",
    phone: undefined,
  },
} as const;

const siteConfigs: Record<Locale, SiteConfig> = {
  fr: {
    ...baseSiteConfig,
    description:
      "Portfolio de Yoann Andrieux, Dev React Native — applications mobiles, produits React/Next.js et culture UX",
    locale: "fr-FR",
    seo: {
      title: "Dev React Native | Portfolio Yoann Andrieux",
      description:
        "React Native en production, produits React/Next.js, architecture, UX et livraison : découvrez le parcours et les études de cas de Yoann Andrieux.",
      keywords: [
        "React Native",
        "iOS",
        "Android",
        "Développeur mobile",
        "TypeScript",
        "Next.js",
        "Node.js",
        "Freelance",
        "Animations",
        "UX",
        "Développement application",
        "Développeur React",
        "Portfolio développeur",
      ],
    },
  },
  en: {
    ...baseSiteConfig,
    description:
      "Yoann Andrieux’s React Native Dev portfolio — mobile applications, React/Next.js products and UX craft",
    locale: "en-US",
    seo: {
      title: "React Native Dev | Yoann Andrieux Portfolio",
      description:
        "Production React Native, React/Next.js products, architecture, UX and delivery through Yoann Andrieux’s work and case studies.",
      keywords: [
        "React Native",
        "iOS",
        "Android",
        "Mobile Developer",
        "TypeScript",
        "Next.js",
        "Node.js",
        "Freelance",
        "Animations",
        "UX",
        "App Development",
        "React Developer",
        "Developer Portfolio",
      ],
    },
  },
};

export const siteConfig: SiteConfig = siteConfigs.fr;

export function getSiteConfig(locale: Locale): SiteConfig {
  return siteConfigs[locale] ?? siteConfigs.fr;
}

/**
 * Textes de l'interface utilisateur
 */
const uiTextsByLocale = {
  fr: {
    nav: {
      home: "Accueil",
      experiences: "Projets",
      skills: "Compétences",
      resume: "CV",
      contact: "Contact",
    },
    sections: {
      explorer: "Explorer",
      techStack: "Technologies utilisées",
      toolsEnvironment: "Outils & Environnement",
      sendMessage: "Envoyez-moi un message",
      highlights: "Points forts",
      keyPoints: "Points clés",
      toolsUsed: "Outils utilisés",
      technicalSkills: "Compétences techniques",
      softSkills: "Soft skills",
      experience: "Expérience",
      career: "Parcours",
      education: "Formation",
      experienceAndEducation: "Parcours & Formation",
    },
    stackLabels: {
      frontend: "Frontend",
      backend: "Backend",
      database: "Database",
      devops: "DevOps",
      testing: "Testing",
    },
    buttons: {
      viewProject: "Voir le projet",
      viewCode: "Voir le code",
      viewDetails: "Voir détails",
      viewWebsite: "Voir le site",
      send: "Envoyer",
      back: "Retour",
      retry: "Réessayer",
      viewMyExperiences: "Voir les études de cas",
      contactMe: "Me contacter",
    },
    labels: {
      present: "Présent",
      privateExperience: "Confidentiel",
      allRightsReserved: "Tous droits réservés.",
      availableForProjects: "Ouvert aux échanges",
    },
    hero: {
      swipeToNavigate: "Faites glisser pour naviguer • Cliquez sur les onglets",
      letsWorkTogether: "Travaillons ensemble",
      projectQuestion: "Un poste, une mission ou un produit à me présenter ?",
      letsDiscuss: "Échangeons sur le contexte et les enjeux.",
    },
    descriptions: {
      experiencesSubtitle: "Une sélection de mes expériences web et mobile",
      myTechStack: "Mon stack technique et mes expertises",
      myProfessionalBackground: "Mon parcours professionnel",
      aiToolsIntegrated: "Outils IA intégrés au workflow",
    },
    form: {
      name: "Nom",
      namePlaceholder: "Votre nom",
      email: "Email",
      emailPlaceholder: "votre@email.com",
      message: "Message",
      messagePlaceholder: "Présentez le poste, la mission ou le problème produit…",
    },
    messages: {
      messageSent: "Message envoyé !",
      messageSentDescription: "Je vous répondrai dans les plus brefs délais.",
      willReplyShort: "Je vous répondrai bientôt",
      pleaseWaitTitle: "Veuillez patienter",
      pleaseWaitDescription:
        "Merci de patienter quelques secondes avant de renvoyer un message.",
      errorTitle: "Erreur",
      genericError: "Une erreur est survenue",
      cannotSend: "Impossible d'envoyer le message",
    },
    stats: {
      team: "Équipe",
      downloads: "Repère",
      platforms: "Plateformes",
      publishedApps: "applications publiées",
      stackExpertise: "Stack technique & expertise",
      discussProject: "Discutons de votre projet",
    },
    filters: {
      all: "Tous",
      dev: "Dev",
      cdi: "CDI",
      freelance: "Freelance",
      personal: "Perso",
      ponctuel: "Mission",
      hors_tech: "Production",
      cinema: "Cinéma",
      ops: "Management Ops",
    },
    download: {
      pageTitle: "Ressources",
      pageSubtitle: "Photos de profil, bannières LinkedIn et assets pour les réseaux sociaux",
      sectionBanners: "Bannières LinkedIn",
      sectionPortraitRaw: "Portrait brut",
      sectionPortrait: "Photos de profil — Portrait",
      sectionPortraitHalo: "Style héros — Liseré + Halo",
      sectionPortraitSolid: "Fonds unis",
      sectionCircle: "Photos de profil — Médaillon",
      sectionCircleHalo: "Halo + fond coloré",
      sectionCircleSolid: "Fonds unis",
      sectionCirclePlain: "Simples — Sans liseré",
      sectionCircleBustHalo: "Cadrage buste — Liseré + Halo",
      sectionCircleBust: "Cadrage buste — Fonds unis",
      sectionSocial: "Réseaux sociaux & autres",
      sectionDocuments: "Documents",
      previewButton: "Prévisualiser",
      previewTitle: "Aperçu bannière",
      downloadButton: "Télécharger",
      downloading: "Téléchargement...",
      formatLabel: "Format",
      dimensionsLabel: "Dimensions",
    },
  },
  en: {
    nav: {
      home: "Home",
      experiences: "Work",
      skills: "Skills",
      resume: "Resume",
      contact: "Contact",
    },
    sections: {
      explorer: "Explore",
      techStack: "Tech stack",
      toolsEnvironment: "Tools & Environment",
      sendMessage: "Send me a message",
      highlights: "Highlights",
      keyPoints: "Key points",
      toolsUsed: "Tools used",
      technicalSkills: "Technical skills",
      softSkills: "Soft skills",
      experience: "Experience",
      career: "Career Path",
      education: "Education",
      experienceAndEducation: "Experience & Education",
    },
    stackLabels: {
      frontend: "Frontend",
      backend: "Backend",
      database: "Database",
      devops: "DevOps",
      testing: "Testing",
    },
    buttons: {
      viewProject: "View project",
      viewCode: "View code",
      viewDetails: "View details",
      viewWebsite: "View website",
      send: "Send",
      back: "Back",
      retry: "Try again",
      viewMyExperiences: "View case studies",
      contactMe: "Contact me",
    },
    labels: {
      present: "Present",
      privateExperience: "Confidential",
      allRightsReserved: "All rights reserved.",
      availableForProjects: "Open to conversations",
    },
    hero: {
      swipeToNavigate: "Swipe to navigate • Tap the tabs",
      letsWorkTogether: "Let's work together",
      projectQuestion: "A role, engagement or product you would like to discuss?",
      letsDiscuss: "Let’s discuss the context and constraints.",
    },
    descriptions: {
      experiencesSubtitle: "A selection of my web and mobile experiences",
      myTechStack: "My tech stack and expertise",
      myProfessionalBackground: "My professional background",
      aiToolsIntegrated: "AI tools integrated into workflow",
    },
    form: {
      name: "Name",
      namePlaceholder: "Your name",
      email: "Email",
      emailPlaceholder: "you@email.com",
      message: "Message",
      messagePlaceholder: "Describe the role, engagement or product problem…",
    },
    messages: {
      messageSent: "Message sent!",
      messageSentDescription: "Thanks! I'll get back to you shortly.",
      willReplyShort: "I'll reply soon",
      pleaseWaitTitle: "Please wait",
      pleaseWaitDescription:
        "Please wait a few seconds before sending another message.",
      errorTitle: "Error",
      genericError: "Something went wrong",
      cannotSend: "Couldn't send the message",
    },
    stats: {
      team: "Team",
      downloads: "Evidence",
      platforms: "Platforms",
      publishedApps: "apps shipped",
      stackExpertise: "Tech stack & expertise",
      discussProject: "Let's talk about your project",
    },
    filters: {
      all: "All",
      dev: "Dev",
      cdi: "Full-time",
      freelance: "Freelance",
      personal: "Personal",
      ponctuel: "Contract",
      hors_tech: "Production",
      cinema: "Cinema",
      ops: "Management Ops",
    },
    download: {
      pageTitle: "Resources",
      pageSubtitle: "Profile photos, LinkedIn banners and social media assets",
      sectionBanners: "LinkedIn Banners",
      sectionPortraitRaw: "Raw Portrait",
      sectionPortrait: "Profile Photos — Portrait",
      sectionPortraitHalo: "Hero Style — Stroke + Halo",
      sectionPortraitSolid: "Solid Backgrounds",
      sectionCircle: "Profile Photos — Medallion",
      sectionCircleHalo: "Halo + colored background",
      sectionCircleSolid: "Solid Backgrounds",
      sectionCirclePlain: "Plain — No stroke",
      sectionCircleBustHalo: "Bust crop — Stroke + Halo",
      sectionCircleBust: "Bust crop — Solid backgrounds",
      sectionSocial: "Social Media & Others",
      sectionDocuments: "Documents",
      previewButton: "Preview",
      previewTitle: "Banner preview",
      downloadButton: "Download",
      downloading: "Downloading...",
      formatLabel: "Format",
      dimensionsLabel: "Dimensions",
    },
  },
} as const;

export const uiTexts = uiTextsByLocale.fr;

export function getUiTexts(locale: Locale) {
  return uiTextsByLocale[locale] ?? uiTextsByLocale.fr;
}
